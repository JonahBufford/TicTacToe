...
void TicTacToeWindow::updateScreen()
{
    ...
    if (tttModel->hasSomeoneWon()) {
    ...
    else if (tttModel->getTurnCounter() == 9) {
        QString msg = "Draw";
        ui->IBStatusLabel->setText(msg);
        ui->TBStatusLabel->setText(msg);
    }
        ...
        // no winner yet
        tttModel->nextTurn();
        if(tttModel->isAiGame()){
            if(tttModel->isHardAiTurn()){
                QString msg = "AI's turn";
                ui->IBStatusLabel->setText(msg);
                ui->TBStatusLabel->setText(msg);
                tttModel->hardAiTurn();
                updateScreen();
            }
            else if(tttModel->isEasyAiTurn()){
                QString msg = "AI's turn";
                ui->IBStatusLabel->setText(msg);
                ui->TBStatusLabel->setText(msg);
                tttModel->easyAiTurn();
                ...
                QString msg = "Your turn";
                ui->IBStatusLabel->setText(msg);
                ui->TBStatusLabel->setText(msg);
            }
        }
        else{
            QString msg = tttModel->whosWinner() + "'s turn";
            ui->IBStatusLabel->setText(msg);
            ui->TBStatusLabel->setText(msg);
        }
    }
...
void TicTacToeWindow::on_aiHardButton_clicked()
{
    tttModel->reset();
    tttModel->setTurn('X');
    tttModel->setHardAi();
...
void TicTacToeWindow::on_aiEasyButton_clicked()
{
    tttModel->reset();
    tttModel->setEasyAi();
    updateScreen();
